...
void keyPressed() {
  ...
  if (LEGAL_KEY && squares[pos.x-1][pos.y-1].moveable == true) {
    ...
    if      (operations[0] == "+") { value += squares[pos.x-1][pos.y-1].number; }
      ...
      catch (ArithmeticException e) { background(255); txt("YOU DIVIDED BY ZERO!\n Your final score: Undefined", 10, screen.width/2, screen.height/2); noLoop(); }
    } 
  ...
  if (LEGAL_KEY) {
    ...
    if (cont == false) {
      background(255);
      txt("There are no more squares.\n Your final score: " + value, 10, screen.width/2, screen.height/2);
      noLoop();
    }
  }
}
...
void draw() {
  ...
  txt("Operations: (" + join(operations, ", ") + "). Current Operation: \"" + operations[0] + "\"", 10, screen.width/4, 32);
  txt("Value: " + value, 10, screen.width/4, 64);
  ...
  for (int xi = pos.x-1; xi <= pos.x+1; xi++) {
    for (int yi = pos.y-1; yi <= pos.y+1; yi++) {
      ...
      try { squares[xi-1][yi-1].show(new Point(x, y), (screen.width/2)/3, 10); }
      catch (ArrayIndexOutOfBoundsException e) {  }
      y += (screen.height-TEXT_SPACE)/3;
    }
    x += (screen.width/2)/3;
    y = TEXT_SPACE;
  }
  ...
  stroke(0);
  int w = (screen.width/2)/BOARD_HEIGHT, h = (screen.height-TEXT_SPACE)/BOARD_HEIGHT;
  for (int i = 0; i < squares.length; i++) {
    for (int ii = 0; ii < squares[0].length; ii++) {
      if (pos.x-1 == i && pos.y-1 == ii) { fill(0); rect(i*w+(screen.width/2), ii*h+TEXT_SPACE, w, h); noFill(); }
      else if (squares[i][ii].moveable == false) { fill(102, 255, 102); rect(i*w+(screen.width/2), ii*h+TEXT_SPACE, w, h); noFill(); }
      else { fill(255, 51, 51); rect(i*w+(screen.width/2), ii*h+TEXT_SPACE, w, h); noFill(); }
    }
  }
  noFill();
}
